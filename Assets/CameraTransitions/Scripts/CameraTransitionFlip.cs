...
namespace Ibuprogames
{
  namespace CameraTransitionsAsset
  {
    ...
    /// Flip transition.
    /// </summary>
    public sealed class CameraTransitionFlip : CameraTransitionBase
    {
      /// <summary>
      /// Flip transition.
      /// </summary>
      ...
      public override void SetParameters(object[] parameters)
      {
        if (parameters.Length == 1 && parameters[0].GetType() == typeof(CameraTransitionFlip.Modes))
          Mode = (CameraTransitionFlip.Modes)parameters[0];
        else
          Debug.LogWarning(@"[Ibuprogames.CameraTransitions] Effect 'Flip' required parameters: mode (CameraTransitionFlip.Modes).");
      }
